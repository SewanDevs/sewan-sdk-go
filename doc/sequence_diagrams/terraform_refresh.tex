...
\begin{center}
  \begin{tikzpicture}
	...
	\begin{umlcall}[op=3.\ createSchema(localResourceState), type=asynchron]{terraform}{schema}
	  \begin{umlcall}[op=4.\ resourceRead(), type=synchron, return=distantResourceState]{terraform}{sdk}
	  ...
	  \end{umlcall}
	  \begin{umlcall}[op=7.\ updateResource(schema), type=synchron, return=OK]{terraform}{localstate}
	  \end{umlcall}
	\end{umlcall}
      \end{umlfragment}
